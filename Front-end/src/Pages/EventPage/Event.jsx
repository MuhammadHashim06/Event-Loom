...
export default function Event() {
  ...
  useEffect(() => {
    ...
    fetchEvents();
  }, [token]); // Token dependency to re-fetch if token changes
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/tickets",
        {
          ...
          userId: userData.userId, // Get userId from session storage
        },
        {
          headers: {
            Authorization: `Bearer ${token}`, // Include the token in the request header
      ...
      // Optionally fetch updated tickets for the user if needed
    } catch (error) {
